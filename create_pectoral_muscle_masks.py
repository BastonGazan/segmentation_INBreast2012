...
xml_origin_folder = r'In_Breast_2012\PectoralMuscle\Pectoral_Muscle_XML'
end_folder = r'In_Breast_2012\Pectoral_Muscle_Masks'
...
def cargar_puntos(point_string):
    x,y = tuple([float(i) for i in point_string.strip('()').strip('{').strip('}').split(',')])
    return y,x
...
def generar_mascara(xml_path,dcm_file):
    ...
    if os.path.exists(xml_path):
        with open (os.path.join(xml_path),'rb') as mask_file:
            i = 0
            file_dict = plistlib.load((mask_file), fmt=plistlib.FMT_XML, dict_type=dict)
            if'Images' in file_dict:
                plist_dict = file_dict['Images'][0]
...
def export_mask(dcm_file):
    mask = generar_mascara(os.path.join(xml_origin_folder,str(dcm_file).split('_')[0]+'_muscle.xml'),dcm_file)

    mask_path = os.path.join(end_folder, str(dcm_file).split('_')[0]+'_pectoral_muscle_mask'+'.jpg')
